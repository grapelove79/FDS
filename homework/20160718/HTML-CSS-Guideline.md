...
상대단위 (px, em, ex, %, rem) | 절대단위 (in, cm, mm, pt, pc)
--- | ---
px  in
- pixel을 기준으로 브라우저에는 고정적이지만 표시장치(기기의 해상도)에 따라서 상대적인 크기를 갖는다. - inch 단위로 계산
- 1px = 1dot  - 1inch = 2.54cm
em  cm
- 해당폰트의 대문자 M의 너비를 기준으로 값을 가진다.  - centimeter 단위로 계산
ex  - 1cm = 10mm
- x-height | mm
- 해당폰트의 소문자 x의 높이를 기준으로 값을 가진다.  - 단위로 계산
%  - 10mm = 1cm
- percent | pt
- 기본글꼴의 크기에 대해 상대적으로 값을 가진다.  - point 단위로 계산
rem  - 1pt = 1/72inch
- root em pc
- 최상위 요소의 크기에 대해 상대적으로 값을 가진다.  - pica단위로 계산
						 | - 1pc = 12pt
