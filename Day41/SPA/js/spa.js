(function(global, XHR){
  ...
  // 통신 상태 관찰 이벤트 핸들링
  xhr.onreadystatechange = function() {//XMLHttpRequest가지고 있는 능력을 이용해 통신상태를 관찰해
  ...
  // 총 4개의 a요소에 -1을 하면 마직막 3번째 contact를 불러오고  nav_links[i]에 i에 3이 담기고 i는 1씩 감소(i--)한다.
  for ( var link, i = nav_links.length - 1; nav_links[i]; i-- ) {
    link = nav_links[i];
    // 1. 클로저 + 우회 메소드(proxy, bind) 사용
    link.onclick = (function(index){
      return viewUpdate.bind(link, index);// 매개변수 link를 지정안하면 window를  불려온다. bind()메소드는 Click했을때 실행한다.(call()메소드는 바로 실행 하기 때문에 flas를 반환)
    })(i);
    // 2. 객체.속성 사용
    // link.idx = i;
  ...
  link.onclick();
// console.log(XMLHttpRequest); // XMLHttpRequest() { [native code] } 는 브라우저가 원래 가지고 있는 것(내장), 통신하려고 만들어 둔것.
})(this, this.XMLHttpRequest || this.ActiveXObject('Microsoft.XMLHTTP'));

//XMLHttpRequest가 실행되는 모던(최신)브라우저에서 실행되니?