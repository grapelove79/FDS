...
(function(global, XHR){
  ...
  // 비동기 통신을 수행할 객체를 생성
  var xhr = createXHR(); // 위 함수에서 반환된 값 XMLHttpRequest === xhr인스턴스가 createXHR()담겨 즉시 실행되고 xhr변수에 담는다.
  //console.log(xhr);
  ...
  var more_btn = document.querySelector('.button');

  call_ajax_btn.onclick = updateViewPlace;
  more_btn.onclick = updateViewPlace;
  ...
  xhr.onreadystatechange = function() {
    ...
    if ( this.status === 200 && this.readyState === 4 ) {
      ...
      var person = {};
      var template = '<li>'; //template화면에 뿌려주기 위한 것
      // people 반복 순환 처리
